...
export default function ChallengeDetailScreen() {
  ...
  const totalDeposited = challenge.total_deposited_sol ?? 0;
  const requiredTotal = challenge.prize_model === 'A'
    ? (challenge.prize_pool ?? 0)
    : challenge.prize_model === 'B'
    ? (joined * (challenge.prize_pool ?? 0))
    : challenge.prize_model === 'C'
    ? (2 * (stakeAmt ?? 0))
    : 0;
        ...
        <Animated.View style={{ opacity: heroOpacity, transform: [{ translateY: heroY }] }}>
          <LinearGradient colors={[catCol + '30', C.bg]} style={s.hero}>
            ...
            <View style={s.heroPills}>
              ...
              {showDeposits && (
                <View style={[s.stakePill, { borderColor: (depositsOk ? C.green : C.orange) + '55', backgroundColor: (depositsOk ? C.green : C.orange) + '15' }]}>
                  <Text style={s.stakeEmoji}>🔒</Text>
                  <Text style={[s.stakeAmt, { color: depositsOk ? C.green : C.orange }]}>◎{totalDeposited.toFixed(2)}/{requiredTotal.toFixed(2)} FUNDED</Text>
                </View>
              )}
              {isMulti && (
